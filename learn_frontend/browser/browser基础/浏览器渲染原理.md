...
由于预解析线程的存在，style 和 link 标签的下载都是在预线程当中进行的，大部分情况不会阻塞主线程的解析。如果主线程解析到`link`位置，此时外部的 CSS 文件还没有下载解析好，主线程也不会等待，继续解析后续的 HTML。这是因为下载和解析 CSS 的工作是在预解析线程中进行的。这就是 CSS 不会阻塞 HTML 解析的根本原因。
